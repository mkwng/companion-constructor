...
import { AttributeSelection, Companion, Layer, LayerWithData, Pose, RGBColor } from "../data/types";
...
const translateImage = (source: HTMLImageElement | HTMLCanvasElement, x: number, y: number): HTMLCanvasElement => {
	const tempCanvas = document.createElement("canvas");
	tempCanvas.width = source.width;
	tempCanvas.height = source.height;
	const tempCtx = tempCanvas.getContext("2d");
...
const rotateImage = (source: HTMLImageElement | HTMLCanvasElement, angle: number): HTMLCanvasElement => {
	const tempCanvas = document.createElement("canvas");
	tempCanvas.width = source.width;
	tempCanvas.height = source.height;
	const tempCtx = tempCanvas.getContext("2d");
...
const applyTransform = (source: HTMLImageElement | HTMLCanvasElement, pose: Pose): HTMLCanvasElement | HTMLImageElement => {
	switch (pose) {
		case 1:
	...
	useEffect(() => {
		...
		async function load() {
			...
			for (let i = 0; i < layersWithData.length; i++) {
				...
				if (layer.path == "/attributes/pose1/00-background/bg-v_background.png" && hideBackground) {
					continue;
				}
				if (!active) {
					return;
			...
			{companion && (
				<div className={`max-w-full mx-auto z-10 relative ${maxHeight && "max-h-2/3-screen"}`}>
					<canvas
						...
						height={h}
						ref={canvasRef}
						className={`max-w-full mx-auto transition-opacity ${maxHeight && "max-h-2/3-screen"} ${
